...
for (i in gene) {
  ...
  # 1) get genotype order + stats from df.out (first matching row)
  rowi <- df.out %>% filter(snp == sp, variable == i) %>% slice_head(n = 1)
  
  levs  <- c(rowi$geno_0, rowi$geno_1, rowi$geno_2)
  P.VAL <- rowi$p.value_un
  BETA  <- rowi$beta_un
  ...
  # 4) plot
  p <- ggplot(l.plot2, aes(x = .data[[sp]], y = .data[[i]], fill = .data[[sp]])) +
